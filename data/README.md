...
这几个数据源的官网有反爬验证或仅以年度报告形式发布，无法稳定自动抓取，
由运营者从官方发布页**手工更新**到这里的 JSON 文件。接口会自动读取，
文件缺失或为空时页面显示"尚未配置"，不会展示编造数据。

## 推荐：直接在后台页面编辑（无需登服务器）

打开 `admin.html` → 输入 ADMIN_TOKEN → 页面底部「**运营数据维护**」，
选择数据文件即可表格化增删改，保存后华贸雷达刷新即生效。
后台会做格式校验（月份 YYYY-MM、评级 1-9、运价为数字等），
填错会提示具体行号，不会写入非法内容。

下面的 JSON 结构说明供直接改文件时参考。
